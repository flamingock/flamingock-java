...
- **Cleanup: two-dimensional state model (2026-05-26).** Formalized the operation × planner split and removed everything the cleaner model no longer needs.
  ...
  - **Always-walk: no skip on operation-touched stages (2026-05-27).** Removed the `state.isNotStarted()` early-return in `stampSnapshotFacts`. The planner now walks every stage on every iteration regardless of state. Defensive merge in `markStageAlreadyAppliedFromAudit` (skips IDs the operation already recorded) plus monotone semantics in `markStageVerdict` guarantee no overwrite of operation-written facts. Motivation: when the audit knows applies the operation doesn't (CLI `mark-as-applied` during a run, third-party audit-tool writes, the upcoming parallel-stage feature where multiple instances apply different changes from the same stage concurrently), the report now surfaces those via planner-added `ALREADY_APPLIED` records on the operation-completed stage. The stage's state stays whatever the operation set; only the per-change records get enriched.
  - **Explicit `NOT_REACHED` per change + reporter rewording (2026-05-27).** Every loaded change now starts life with `ChangeStatus.NOT_REACHED` at `PipelineRun` construction (`StageRun` constructor populates one record per loaded change). Writers transition records forward as they learn facts: operation's `markStageCompleted` / `markStageFailed` switched from full-list **replace** to **merge by change ID** (executor's records win for IDs it processed; records for unprocessed changes survive as NOT_REACHED). Planner's `markStageAlreadyAppliedFromAudit` switched from "skip if exists" to **upgrade-if-NOT_REACHED** (only replaces records still at NOT_REACHED; never touches operation-written positive records). At end-of-run every record carries a meaningful status; the previously-dead `ChangeStatus.NOT_REACHED` enum value is now load-bearing. Aggregate counter `notReachedChanges` added to `ExecuteResponseData`; reporter changes line gains `M not reached` so math invariant `A + S + F + M == N` always closes. Reporter wording rationalized: `applied` → `newly applied`; `already at target state` → `already applied`. Summary one-liner mirrors: `newly_applied=A, already_applied=S, failed=F, not_reached=M`. Resolves the mid-stage-failure gap (executor stops after a failure → trailing unprocessed changes now appear explicitly as NOT_REACHED in both per-change list and aggregate counter).
