...
class SteppableTemplateExecutableChangeTest {
    ...
    void shouldApplyAllStepsInOrder() {
        ...
        SteppableTemplateExecutableChange<TemplateVoid, TemplateString, TemplateString> change = new SteppableTemplateExecutableChange<>(
                "test-stage",
                mockDescriptor,
                ChangeAction.APPLY,
                applyMethod,
    ...
    void shouldRollbackFromFailedStepInReverseOrder() {
        ...
        assertThrows(ChangeExecutionException.class, () -> change.apply(mockRuntime));
    ...
    void shouldHandleEmptyStepsList() {
        ...
        assertDoesNotThrow(() -> change.apply(mockRuntime));
        assertEquals(0, appliedPayloads.size());

        // Rollback should also work with empty list
        assertDoesNotThrow(() -> change.rollback(mockRuntime));
