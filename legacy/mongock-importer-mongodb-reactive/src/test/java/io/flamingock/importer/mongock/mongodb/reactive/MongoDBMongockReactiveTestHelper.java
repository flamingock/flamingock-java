...
public class MongoDBMongockReactiveTestHelper implements MongockTestHelper {

    private final MongoCollection<Document> changeLogCollection;

    public MongoDBMongockReactiveTestHelper(MongoCollection<Document> changeLogCollection) {
        this.changeLogCollection = changeLogCollection;
    }

    @Override
    public void reset() {
        PublisherSync.complete(changeLogCollection.drop());
    }

    public void write(MongockChangeEntry entry) {
        PublisherSync.complete(changeLogCollection.insertOne(convertToDocument(entry)));
    }
    ...
    public int writeAll(List<MongockChangeEntry> entries) {
        ...
        PublisherSync.complete(changeLogCollection.insertMany(documents));
