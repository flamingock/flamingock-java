...
/**
 * Command to get detailed information about a specific audit issue.
 */
@Command(
        name = "get",
        aliases = "describe",
        description = "Get detailed information about an audit issue",
        mixinStandardHelpOptions = true
)
public class GetIssueCommand implements Callable<Integer> {
    ...
    @Option(names = {"-c", "--change-id"},
            description = "The change ID to inspect (if not provided, shows first issue)")
    ...
    public GetIssueCommand(CommandExecutor commandExecutor) {
    ...
    public Integer call() {
        ...
        CommandResult<IssueGetResponseData> result = commandExecutor.execute(
                ...
                OperationType.ISSUE_GET,
                IssueGetResponseData.class,
                options
        );

        if (result.isSuccess()) {
            IssueGetResponseData data = result.getData();
            if (json) {
                JsonFormatter.print(data);
            } else {
                displayIssue(data, quiet);
            }
            return 0;
        } else {
    ...
    private void displayIssue(IssueGetResponseData data, boolean quiet) {
        if (data == null || !data.isFound()) {
            if (changeId != null) {
                ConsoleFormatter.printInfo("No issue found for change ID: " + changeId);
            } else {
        ...
        System.out.println();
        IssueFormatter.printDetail(data, guidance);
    }
